...
if( !class_exists( 'Easy_Customizer' ) ){
	class Easy_Customizer extends Easy_Customizer_Helper{
		...
		*/
		public static function add_custom_control( $control ){
			...
			if( !is_array( $control ) ){
				$err = esc_html__( 'Invalid argument passed for Custom Control. Must be an array with "type" & "class" key.', 'easy-wordpress-customizer' );
				self::add_error( $err );
				return;
			}
			...
			if( !isset( $control[ 'type' ] ) ){
				$err = esc_html__( 'Custom Control must contain "type" key ( type of control ).', 'easy-wordpress-customizer' );
				self::add_error( $err );
				$flag = false;
			}

			if( !isset( $control[ 'class' ] ) ){
				$err = esc_html__( 'Custom Control must contain "class" key ( name of custom class ).', 'easy-wordpress-customizer' );
				self::add_error( $err );
				$flag = false;
			}		

			if( !isset( $control[ 'sanitize' ] ) ){
				$err = esc_html__( 'Custom Control must contain "sanitize" key ( a callback function ).', 'easy-wordpress-customizer' );
				self::add_error( $err );
				$flag = false;
			}
			...
			foreach( $args[ 'fields' ] as $field ){
				...
				if( isset( $field[ 'id' ] ) ){
					...
					if( array_key_exists( $field_id, self::$settings ) ){
						...
						$msg = sprintf( esc_html__( 'Field with ID: %s already exists.', 'easy-wordpress-customizer' ), $field[ 'id' ] );
						self::add_error( $msg );
					}
		...
		*/
		private static function validate_argument( $args ){
			...
			if( !is_array( $args ) ){
				...
				$msg = esc_html__( 'You must pass array as an argument in set method.', 'easy-wordpress-customizer' );
			...
			if( !isset( $args[ 'section' ] ) ){
				...
				$msg = esc_html__( 'No section found.', 'easy-wordpress-customizer' );
				self::add_error( $msg, 2 );
			...
			if( !is_array( $args[ 'section' ] ) ){
				...
				$msg = sprintf( 
					...
					esc_html__( 'Invalid argument', 'easy-wordpress-customizer' ),	
					esc_html__( 'section', 'easy-wordpress-customizer' ),
					esc_html__( 'Must be a an array with atleast an id, title...', 'easy-wordpress-customizer' )
				);
				...
				$msg = esc_html__( 'Section id is missing.', 'easy-wordpress-customizer' );
				self::add_error( $msg, 2 );
			}

			if( !isset( $args[ 'fields' ] ) ){

				$msg = esc_html__( 'Fields are missing.', 'easy-wordpress-customizer' );
				self::add_error( $msg, 2 );
				...
				$msg = sprintf( 
					'%s <strong><em>%s</em></strong>. %s',
					esc_html__( 'Invalid argument', 'easy-wordpress-customizer' ),	
					esc_html__( 'fields', 'easy-wordpress-customizer' ),
					esc_html__( 'Must be a an array', 'easy-wordpress-customizer' )
				);
						...
						<p>
							<?php  
								echo sprintf( '<strong>%s</strong> %s', 
									esc_html__( 'Easy Customizer:', 'easy-wordpress-customizer' ),
									esc_html( $value ) 
								);  
							?>
						</p>
		...
		private static function get_panel_arg( $panel ){
			...
			$args = array(
			    'title' => empty( $panel[ 'title' ] ) ?
			     esc_html__( 'No Title Specified.', 'easy-wordpress-customizer' ) : 
			     esc_html__( 'Theme Options','easy-wordpress-customizer' ),
			);
		...
		*/
		private static function get_section_arg( $panel_id = false, $section ){
			$args = array(
				'title' => empty( $section[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'easy-wordpress-customizer' ) : $section[ 'title' ],
			);
...
add_action( 'customize_register', array( Easy_Customizer::get_instance(), 'register' ) );

if( !function_exists( 'easy_customizer_get' ) ):
	/**
	 ...
	 * @package Easy WordPress Customizer
	 */
	function easy_customizer_get( $id ){
		$id = Easy_Customizer_Helper::with_prefix( $id );
		return Easy_Customizer::get( $id );
	}
